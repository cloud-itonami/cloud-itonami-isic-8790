(ns residential.operation
  "OperationActor -- one residential-care operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (ResidentialOps-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Residential Care Governor
  ...
  One graph run = one residential-care operation (intake -> advise ->
  ...
  decision to a human operator (licensed residential-care staff). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:actuation/finalize-care-plan`/`:actuation/finalize-
  incident-response` ALWAYS reach this node when the governor is
  clean -- see `residential.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [residential.residentialadvisor :as residentialadvisor]
...
(defn build
  ...
  `residential.store/Store`).
  opts:
    :advisor      -- a `residential.residentialadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (residentialadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; ResidentialOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (residentialadvisor/-advise advisor store request)]
            {:proposal p :audit [(residentialadvisor/trace request p)]})))

      ;; Residential Care Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
