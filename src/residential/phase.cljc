(ns residential.phase
  "Phase 0->3 staged rollout -- the residential-care analog of
  ...
  milestone still to come. Finalizing a real care plan and finalizing
  a real incident response are the two real-world residential-care
  acts this actor performs; both are always a human licensed-staff
  call. `residential.governor`'s high-stakes gate enforces the same
  invariant independently -- two layers, not one, agree on this.
  `:safeguarding/screen`/`:background-check/screen` are likewise
...
(def read-ops  #{})
(def write-ops #{:resident/intake :careplan/verify :safeguarding/screen
                 :background-check/screen
                 :actuation/finalize-care-plan :actuation/finalize-incident-response})

;; NOTE the invariant: `:actuation/finalize-care-plan`/`:actuation/
...
(def phases
  ...
  {0 {:label "read-only"        :writes #{}                                                       :auto #{}}
   ...
   2 {:label "assisted-verify"  :writes #{:resident/intake :careplan/verify :safeguarding/screen
...
(defn verdict->disposition
  "Map a Residential Care Governor verdict to a base disposition
